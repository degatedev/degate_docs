# Create Wallet

**Creating your DeGate account**

There are a couple of ways to create your DeGate account

*   Download the DeGate App from App Store or Google Play, or download the APK file.

    [ ![download\_app\_store.png](https://hub.degate.com/common/wallet-setup-img-en/Download_AppStore.png)](https://apps.apple.com/app/apple-store/id6742168343?pt=127643354\&ct=docs\&mt=8) [ ![download\_google\_play.png](https://hub.degate.com/common/wallet-setup-img-en/Download_GooglePlay.png)](https://play.google.com/store/apps/details?id=com.app.degate\&referrer=utm_source%3Ddocs%26utm_medium%3Ddocs) [![download\_apk.png](https://hub.degate.com/common/wallet-setup-img-en/Download_Android.png)](https://download.degate.com/apps/degate-android.apk)
* Sign up via email address on [app.degate.com](https://app.degate.com)\
  → Email login is powered by privy

<figure><img src="../.gitbook/assets/Screenshot 2025-04-09 at 9.53.45 PM.png" alt="" width="375"><figcaption></figcaption></figure>

* Connect via Web3 wallet on [app.degate.com](https://app.degate.com)\
  → Installed browser wallets will be displayed, simply select your preferred wallet to connect.

<figure><img src="../.gitbook/assets/Screenshot 2025-04-09 at 9.51.35 PM.png" alt="" width="375"><figcaption></figcaption></figure>
